...
from app.models import Post,Comments,Subscribe,Tag,Profile,WebsiteMeta
...
def index(request):
    ...
    if request.POST:
        ...
        if subscribe_form.is_valid():
            ...
            subscribe_successful = 'Your Subscription Mail has been Receive'
            subscribe_form = SubscribeForm()
    ...
    context = {'posts':posts,'top_posts':top_posts,'recent_posts':recent_posts,
               ...
               'featured_blog':featured_blog,'website_info':website_info}
...
def post_page(request,slug):
    ...
    if post.likes.filter(id=request.user.id).exists():
        liked = True
    number_of_likes = post.number_of_like()
    post_is_liked = liked
    ...
    context = {'post':post,'form':form,
               ...
               "recent_posts":recent_posts,'top_author':top_author,'tags':tags,
...
def author_page(request,slug):
    ...
    top_posts = Post.objects.filter(author=profile.user).order_by('-view_count')
...
@login_required
def logout(request):
    keys_to_delete = list(request.session.keys())  # Create a list of keys before iterating
    for sessionkey in keys_to_delete:
